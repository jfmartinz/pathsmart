...
export default function MessageInbox() {
  ...
  const renderSidebar = () => (
    ...
    >
      ...
      >
        <Image source={require('../../../assets/map.png')} style={styles.logoImage} />
      ...
      >
        <Image source={require('../../../assets/account-creation.png')} style={styles.logoImage} />
        {sidebarExpanded && <Text style={styles.menuText}>Account Creation</Text>}
      </TouchableOpacity>
      <TouchableOpacity
        style={[styles.menuItem, activeTab === 'dashboard' && styles.activeMenuItem]}
        onPress={toggleProductSubmenu}
      >
        <Image source={require('../../../assets/PnS.png')} style={styles.logoImage} />
        {sidebarExpanded && (
          <View style={styles.productServicesContainer}>
            ...
            <Image
              ...
              style={[styles.arrowIcon, { transform: [{ rotate: showProductSubmenu ? '0deg' : '270deg' }] }]}
            />
          </View>
        )}
      </TouchableOpacity>
      {sidebarExpanded && showProductSubmenu && (
        <View style={styles.submenu}>
          <TouchableOpacity
            ...
            onPress={() => router.push('/modules/stallManagement/screens/qualityGuide')}
          >
            <Text style={styles.submenuText}>Quality Guide</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={styles.submenuItem}
            onPress={() => router.push('/modules/stallManagement/screens/listing')}
          >
      ...
      <View style={styles.sidebarBottom}>
        <TouchableOpacity
          style={[styles.menuItem, activeTab === 'user-account' && styles.activeMenuItem]}
          onPress={() => {
            setActiveTab('user-account');
            router.push('/modules/stallManagement/screens/account');
          }}
        >
          <Image source={require('../../../assets/user-account.png')} style={styles.logoImage} />
          {sidebarExpanded && <Text style={styles.menuText}>Account</Text>}
        </TouchableOpacity>
        <TouchableOpacity style={styles.menuItem} onPress={handleLogout}>
          <Image source={require('../../../assets/logout.png')} style={styles.logoImage} />
            ...
            {selectedMessage && (
              <ScrollView showsVerticalScrollIndicator={false}>
                <View style={styles.detailHeader}>
                  <View>
                    <Text style={styles.detailOwner}>{getOwnerName(selectedMessage)}</Text>
                    <Text style={styles.detailStall}>
                      {selectedMessage.stall?.stall_name || 'Unknown Stall'}
                    </Text>
                  </View>
                  <View style={[styles.statusBadge, { backgroundColor: getStatusBg(selectedMessage.status) }]}>
                    <Text style={[styles.statusText, { color: getStatusColor(selectedMessage.status) }]}>
                ...
                {selectedMessage.admin_reply ? (
                  <View style={styles.replySection}>
                    ...
                    {selectedMessage.replied_at && (
                      <Text style={styles.replyDate}>{formatDate(selectedMessage.replied_at)}</Text>
                    )}
                  </View>
                ) : (
                  <View style={styles.replyForm}>
                    <Text style={styles.replyFormTitle}>Reply to this request</Text>
                    <TextInput
                    ...
                    <View style={styles.replyActions}>
                      ...
                      <TouchableOpacity
                        style={[styles.sendReplyBtn, replying && { opacity: 0.6 }]}
                        onPress={handleReply}
                        disabled={replying}
                      >
...
const styles = StyleSheet.create({
  ...
  sidebar: {
    width: 60, backgroundColor: '#1976D2', paddingVertical: 20,
  ...
  submenu: {
    width: '100%', backgroundColor: '#1e88e5', borderRadius: 8,
  ...
  sidebarBottom: { position: 'absolute', bottom: 130, width: '100%', alignItems: 'center' },
  messageMenuRow: { flexDirection: 'row', alignItems: 'center', flex: 1 },
  badge: {
    backgroundColor: '#FF5252', borderRadius: 10, minWidth: 20, height: 20,
    justifyContent: 'center', alignItems: 'center', marginLeft: 8, paddingHorizontal: 5,
  },
  ...
  searchBox: {
    flex: 1, maxWidth: 300, height: 40, borderRadius: 8, borderWidth: 1,
    borderColor: '#ddd', flexDirection: 'row', alignItems: 'center',
    paddingHorizontal: 12, backgroundColor: '#fff', marginLeft: 'auto',
  },
  ...
  messageCardLeft: { flexDirection: 'row', alignItems: 'center', flex: 1, gap: 12 },
  avatarCircle: {
    width: 40, height: 40, borderRadius: 20, backgroundColor: '#1976D2',
    justifyContent: 'center', alignItems: 'center',
  },
  avatarText: { color: '#fff', fontSize: 18, fontWeight: '700' },
  ownerName: { fontSize: 15, fontWeight: '600', color: '#222' },
  ...
  subjectText: { fontSize: 15, fontWeight: '600', color: '#333', marginBottom: 4 },
  ...
  detailSubject: { fontSize: 20, fontWeight: '600', color: '#222', marginTop: 8 },
  ...
  replySection: {
    marginTop: 20, backgroundColor: '#E8F5E9', borderRadius: 10,
    padding: 16, borderLeftWidth: 3, borderLeftColor: '#66BB6A',
  },
  replySectionTitle: { fontSize: 14, fontWeight: '600', color: '#66BB6A', marginBottom: 8 },
  replyBody: { fontSize: 14, color: '#333', lineHeight: 20 },
  replyDate: { fontSize: 12, color: '#aaa', marginTop: 8 },

  replyForm: { marginTop: 24 },
  replyFormTitle: { fontSize: 15, fontWeight: '600', color: '#333', marginBottom: 10 },
  replyInput: {
    borderWidth: 1, borderColor: '#ddd', borderRadius: 8,
    paddingHorizontal: 14, paddingVertical: 10, fontSize: 15,
    backgroundColor: '#F9F9F9', minHeight: 100, textAlignVertical: 'top',
  },
